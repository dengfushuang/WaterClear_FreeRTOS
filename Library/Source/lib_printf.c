...
int fputc(int ch, FILE *f)
{
	...
	do{
		status = UART_GetStatus(UART3, UART_STA_TXBUSY);
		count++;
	} while ((status == SET) && (count != 0x1CE2));
	...
	do{
		status = UART_GetStatus(UART2, UART_STA_TXBUSY);
		count++;
	} while ((status == SET) && (count != 0x1CE2));
	...
	do{
		status = UART_GetStatus(UART1, UART_STA_TXBUSY);
		count++;
	} while ((status == SET) && (count != 0x1CE2));
	...
	do {
		status = UART_GetStatus(UART0, UART_STA_TXBUSY);
		count++;
	} while ((status == SET) && (count != 0x1CE2));
		...
		if (*Data == 0x5c) {                  //'\'
			switch (*++Data) {
				case 'r':                         //回车符
					...
					do {
						Status = UART_GetStatus(UARTx, UART_STA_TXBUSY);
						Count++;
					} while((Status == SET) && (Count != 0x1CE2));
					if(Count == 0x1CE2)
						RET = ERROR;
